...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        loop {
            ...
            if s & (WRITER_HOLDING | WRITER_WAITING) != 0 {
                // writer is holding or waiting(due to writer-priority)
                core::hint::spin_loop();
            } else if s & READER_MASK == READER_MASK {
                // readers are full
                core::hint::spin_loop();
            } else {
                if self
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            let s = self.state.load(Ordering::Acquire);
            if s & WRITER_HOLDING != 0 {
                // another writer is holding
                core::hint::spin_loop();
            } else if s & READER_MASK != 0 {
                // readers are holding
                core::hint::spin_loop();
            } else {
                // the WRITER_WAITING flag might be unset by another writer when it droped,
                // so the current state may be WRITER_WAITING or 0.
                if self
                    .state
                    .compare_exchange(
                        WRITER_WAITING,
...
impl<T> Drop for RwLockWriteGuard<'_, T> {
    fn drop(&mut self) {
        // need to unset WRITER_HOLDING and WRITER_WAITING both.
        self.lock
            .state
